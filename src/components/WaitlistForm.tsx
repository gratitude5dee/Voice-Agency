...
type FormMode = 'waitlist' | 'contact';
...
const WaitlistForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (formMode === 'waitlist') {
        ...
        if (supabaseError) {
          handleSubmissionError(supabaseError);
          return;
        }

        toast.success('Successfully joined the waitlist!');
      } else {
        // Insert data into contact_sales table
        const contactData = {
          name: formData.name,
          email: formData.email,
  ...
  const handleSubmissionError = (supabaseError: any) => {
        ...
        <div className="max-w-3xl mx-auto glass-card rounded-2xl overflow-hidden">
          {!submitted ? <div className="p-8">
              ...
              <div className="flex rounded-lg mb-8 bg-galaxy-purple/20">
                <button 
                  type="button" 
                  className={`flex-1 py-3 text-center rounded-lg transition-all ${formMode === 'waitlist' ? 'bg-galaxy-accent text-white shadow-lg' : 'text-gray-300 hover:text-white'}`} 
                ...
                <button 
                  type="button" 
                  className={`flex-1 py-3 text-center rounded-lg transition-all ${formMode === 'contact' ? 'bg-galaxy-accent text-white shadow-lg' : 'text-gray-300 hover:text-white'}`} 
              ...
              <form onSubmit={handleSubmit}>
                ...
                <button 
                  ...
                  disabled={loading || (formMode === 'contact' && !formData.companyName)}
                >
                  {loading ? (
